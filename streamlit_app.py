...
st.selectbox(label="Bitte wählen Sie ihre Rolle aus:", options=["Mitarbeiter"], index=None, key="selected_role", placeholder="Rolle")
# Deaktiviert: st.checkbox(label="Debug Modus", key="selected_debug_mode")
confirm_button = st.button(label="Bestätigen & Weiter")
if confirm_button:
    if st.session_state.selected_role == "Mitarbeiter":
        st.session_state.role = st.session_state.selected_role
        st.session_state.debug_mode = False if "selected_debug_mode" not in st.session_state else st.session_state.selected_debug_mode
